...
- **[참고]** 하드(Hard) 코딩은 프로그래머가 코드에서 변수값을 리터럴 같이 고정된 값으로 직접 대입해주는 방식.
...
# 😊 배우게 된 점
2022-04-03 - 챕터 1이 끝났다 강의 듣는 내용들은 다 이해가 되는데 손에 익거나 실제로 사용할려면 여러번 만져봐야할것같다 우선은 **지식**을 쌓는 과정이라고 생각하고 하나하나 필기해뒀다가 나중에 틈틈히 꺼내보면서 다시 상기시키면서 **정보**로 만들어야겠다
